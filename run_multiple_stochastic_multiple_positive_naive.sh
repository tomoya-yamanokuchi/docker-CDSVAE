...
do
    ...
    do
        ...
        --config-name=config_cdsvae_dclaw_stochastic_multiple_positive \
        loss.weight.kld_context=1 \
        loss.weight.kld_dynamics=1 \
        loss.weight.contrastive_mi_fx=0.0 \
        loss.weight.contrastive_mi_zx=0.0 \
        loss.weight.mutual_information_fz=0.0 \
        model.network.content.encoder.num_short_clip=$i
    done
done
